...
const url =
  ...
  "&pagination[pageSize]=10";      // 👈 10 items per page (CHANGED)
  ...
  try {
    ...
    const topTen = sortedSections.slice(0, 10);  // CHANGED: now gets top 10

    /* ========================
       Render Featured Main Card (Newest News)
    ========================= */
    if (mainCardContainer && topTen.length > 0) {
      const featuredItem = topTen[0]; // This is guaranteed to be the newest
    ...
    /* ========================
       Render Remaining Latest Items (2nd-10th newest)
    ========================= */
    const remainingItems = topTen.slice(1);  // CHANGED: now includes up to 9 remaining items
    ...
    /* ========================
       Render Ticker News Titles (All top 10)
    ========================= */
    if (tickerContainer) {
      const tickerItems = topTen  // CHANGED: now includes all 10 items
        .map((item) => {
          const title = item.title || "Untitled";
          const docId = item.documentId || "";
          return `
    ...
    console.log("Featured news item details:", {
      title: topTen[0]?.title,
      publish_on: topTen[0]?.publish_on,
      publishedAt: topTen[0]?.publishedAt,
      createdAt: topTen[0]?.createdAt
    });
